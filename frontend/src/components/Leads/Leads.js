...
const Leads = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://lead-management-backend-6ihr.onrender.com/leads/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
...
const onCellValueChanged = async (params) => {
  ...
  try {
    const response = await fetch(`https://lead-management-backend-6ihr.onrender.com/leads/${updatedData._id}`, {
      method: "PUT",
...
,
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        const response = await fetch("https://lead-management-backend-6ihr.onrender.com/leads/", {
          method: "GET",
          credentials: "include",
        });
